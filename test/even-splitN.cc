...
template <typename T> class EvenSplitNTest : public ::testing::Test {
 protected:
  ...
  size_t RandomNumOutputs()
  {
    return std::uniform_int_distribution<size_t>(2, 4)(rng);
  }
...
TEST_F(EvenSplitNTestQS8, define)
{
  ...
  for(size_t i=0;i<num_outputs;++i){
    ...
    ASSERT_EQ(
        xnn_status_success,
        xnn_define_quantized_tensor_value(
        subgraph, xnn_datatype_qint8, signed_zero_point, scale, output_dims[i].size(), output_dims[i].data(), nullptr, 1,
  ...
  ASSERT_EQ(node->type, xnn_node_type_even_split_n);
...
TEST_F(EvenSplitNTestQU8, define)
{
  ...
  for(size_t i=0;i<num_outputs;++i){
    ...
    ASSERT_EQ(
        xnn_status_success,
        xnn_define_quantized_tensor_value(
        subgraph, xnn_datatype_quint8, unsigned_zero_point, scale, output_dims[i].size(), output_dims[i].data(), nullptr, 1,
        /*flags=*/XNN_VALUE_FLAG_EXTERNAL_OUTPUT, &output_id[i]));
    ASSERT_NE(output_id[i], XNN_INVALID_NODE_ID);
  }
  uint32_t output_ddds[num_outputs];
...
TEST_F(EvenSplitNTestQS8, matches_operator_api)
{
  ...
  for(int i=0;i<num_outputs;++i){
    ...
    ASSERT_EQ(xnn_status_success, xnn_reshape_copy_nc_x8(ops[i], batch_size, channels, input_stride, channels, /*threadpool=*/nullptr));
    if(i==0){
      ASSERT_EQ(
      xnn_status_success,
  ...
  for(int i=0;i<num_outputs;++i){
    ...
    ASSERT_EQ(
      xnn_status_success,
      xnn_define_quantized_tensor_value(
        subgraph, xnn_datatype_qint8, signed_zero_point, scale, output_dims[i].size(), output_dims[i].data(), nullptr, i+1,
...
TEST_F(EvenSplitNTestQU8, matches_operator_api)
{
  ...
  for(int i=0;i<num_outputs;++i){
    ...
    if(i==0){
      ASSERT_EQ(
        xnn_status_success,
        xnn_setup_copy_nc_x8(ops[i], input.data(), operator_outputs[i].data()));
    }
    else{
      ASSERT_EQ(
        xnn_status_success,
  ...
  for(int i=0;i<num_outputs;++i){
    ...
    ASSERT_EQ(
      xnn_status_success,
      xnn_define_quantized_tensor_value(
        subgraph, xnn_datatype_quint8, unsigned_zero_point, scale, output_dims[i].size(), output_dims[i].data(), nullptr, i+1,
        /*flags=*/XNN_VALUE_FLAG_EXTERNAL_OUTPUT, &output_ddds[i]));
    ASSERT_NE(output_ddds[i], XNN_INVALID_NODE_ID);
  }
...
TEST_F(EvenSplitNTestF16, matches_operator_api)
{
  ...
  for(int i=0;i<num_outputs;++i){
    ...
    ASSERT_EQ(xnn_status_success, xnn_reshape_copy_nc_x16(ops[i], batch_size, channels, input_stride, channels, /*threadpool=*/nullptr));
    if(i==0){
      ASSERT_EQ(
      xnn_status_success,
      xnn_setup_copy_nc_x16(ops[i], input.data(), operator_outputs[i].data()));
    }
    else{
      ASSERT_EQ(
      xnn_status_success,
      xnn_setup_copy_nc_x16(ops[i], (xnn_float16*) input.data() + (i* (ops[i]->channels)), operator_outputs[i].data()));
    }
    ASSERT_EQ(xnn_status_success, xnn_run_operator(ops[i], /*threadpool=*/nullptr));
  }
...
TEST_F(EvenSplitNTestF32, matches_operator_api)
{
  ...
  for(int i=0;i<num_outputs;++i){
    ...
    ASSERT_EQ(xnn_status_success, xnn_reshape_copy_nc_x32(ops[i], batch_size, channels, input_stride, channels, /*threadpool=*/nullptr));
    if(i==0){
      ASSERT_EQ(
        xnn_status_success,
        xnn_setup_copy_nc_x32(ops[i], input.data(), operator_outputs[i].data()));
    }
    else{
      ASSERT_EQ(
        xnn_status_success,
